...
class ParkingLot(object):
    ...
    def parkThePark(self,slotId, registrationNumner,color):
        '''
        ...
        if len(self.__Tickts) != self.__slotCount:
           self.__Tickts.append(Ticket(slotId, registrationNumner, color))
        else:
           self.__Tickts[slotindex].setRegistratonNumber(registrationNumner)
           self.__Tickts[slotindex].setColor(color)
        self.__slots[slotindex].parkCar()
        if self.__slots[slotindex].checkIfCarParked() == True:
           print "Allocated slot number:",slotId
    ...
    def showParkingStatus(self):
        ...
        for i in range(self.__slotCount):
            if self.__slots[i].checkIfCarParked() == True:
               print str(self.__Tickts[i].getSlotId())+"           "+str(self.__Tickts[i].getregistratonNumber())+"       "+self.__Tickts[i].getcolor()


    def getRegistrationNumbersFoCarsWithColour(self,color):
        '''
        The method is to get registration numbers for parcular color
        ''' 
        cars=[]
        for i in range(self.__slotCount):
            if self.__slots[i].checkIfCarParked() == True and self.__Tickts[i].getcolor() == color:
               cars.append(str(self.__Tickts[i].getregistratonNumber()))

        print ", ".join(cars)

    def getSlotNumbersForCarsWithColour(self,color):
        '''
    ...
    def getSloNumberForRegistrationNumber(self,registartionNumber):
        '''
        ...
        for i in range(self.__slotCount):
            if self.__slots[i].checkIfCarParked() == True and self.__Tickts[i].getregistratonNumber() == registartionNumber:
               ...
               break
        if status == True:
           print self.__Tickts[i].getSlotId()
        else:
           print "Not found"
